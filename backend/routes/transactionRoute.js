...
router.get('/totalAmount', async (req, res) => {
  try {
    ...
    const totalAmount = transactions.reduce((total, transaction) => {
      ...
      if (isNaN(amount)) {
        console.warn(`Invalid amount for transaction ID ${transaction.transactionId}:`, transaction.amount);
        return total;
      }
      return total + amount;
    }, 0);
...
router.get('/averageOrderValue', async (req, res) => {
  try {
    ...
    const averageOrderValue = result.length > 0 ? result[0].averageOrderValue : 0;
...
router.get('/salesByCategory', async (req, res) => {
  try {
    ...
    const salesByCategory = Object.entries(categorySales).map(([category, totalSales]) => ({
      category,
      totalSales
    }));
...
router.get('/salesGrowthRate', async (req, res) => {
  try {
    ...
    const salesGrowthRate = result.map(item => ({
      date: `${item._id.year}-${item._id.month.toString().padStart(2, '0')}-${item._id.day.toString().padStart(2, '0')}`, // Format date as YYYY-MM-DD
      totalSales: item.totalSales
    }));
...
router.get('/topPurchasesProduct', async (req, res) => {
  try {
    const result = await Transaction.aggregate([
      ...
      { $group: { _id: "$items.name", totalPurchases: { $sum: "$items.quantity" } } }, // Group by item name and sum quantities
