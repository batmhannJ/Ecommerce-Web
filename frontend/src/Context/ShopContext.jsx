...
const ShopContextProvider = (props) => {
  ...
  const decreaseItemQuantity = async (productId, selectedSize) => {
    ...
    if (itemIndex !== -1) {
      ...
      setCartItems((prevItems) => {
        const updatedQuantity = prevItems[itemIndex].quantity - 1;

        // Ensure quantity doesn't drop below 1
        if (updatedQuantity < 1) {
          console.warn(
            "Cannot decrease quantity below 1. Item will be removed from the cart."
          );
          return prevItems.filter((_, index) => index !== itemIndex); // Remove item from the cart
        }

        return prevItems.map((cartItem, index) => {
          if (index === itemIndex) {
            return { ...cartItem, quantity: updatedQuantity }; // Decrease the quantity
          }
          return cartItem;
        });
      });
      ...
      try {
        ...
        if (updatedQuantity === 0) return; // Prevent sending a request if quantity goes to zero
